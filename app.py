...
SYNC_INTERVAL = timedelta(seconds=45)
_last_sync = None
...
def _espn_result(ev):
    ...
    home = sides.get("home", {})
    away = sides.get("away", {})
    ht = (home.get("team") or {}).get("displayName")
    at = (away.get("team") or {}).get("displayName")
    return ht, at, ev
...
def _norm_team(name):
    ...
    replacements = {
        ...
        "rcd espanyol":"espanyol", "espanyol barcelona":"espanyol",
        "rcd deportivo":"deportivo de a coruna", "deportivo la coruna":"deportivo de a coruna",
        "deportivo":"deportivo de a coruna", "deportivo de la coruna":"deportivo de a coruna",
        "rc celta":"celta vigo", "celta de vigo":"celta vigo",
        "rcd mallorca":"mallorca", "real betis":"betis", "betis sevilla":"betis",
        "cd alaves":"alaves", "alaves":"alaves", "atletico":"atletico madrid",
        "atletico de madrid":"atletico madrid", "real sociedad san sebastian":"real sociedad",
        "racing":"racing de santander", "racing santander":"racing de santander",
    }
    return replacements.get(s, s)


def sync_fixtures(force=False):
    """Synchronise LaLiga fixtures/results automatically from ESPN's public keyless feed.
    This is best-effort: an external outage never prevents the app from loading.
    """
    global _last_sync
    now = now_local()
    if not force and _last_sync and now - _last_sync < SYNC_INTERVAL:
        return False
    try:
        r = requests.get(
            ESPN_SCOREBOARD_URL,
            params={"dates": "20260801-20270601", "limit": 500},
            timeout=(3, 5),
        )
        r.raise_for_status()
        data = r.json()
        events = data.get("events") or []
        # Do not require an arbitrary minimum number of events: early-season
        # feeds, provider pagination and temporary partial responses are valid.
        if not events:
            raise RuntimeError("La fuente de calendario no devolvió partidos")
    except Exception:
        _last_sync = now
        return False
    ...
    conn = db()
    try:
        # Build the 38 jornadas from the source. Existing user bets remain linked
        # to match ids; when a source match is recognised we update that row.
        existing_rounds = sql(conn, "SELECT * FROM rounds ORDER BY id").fetchall()
        round_by_number = {}
        for rr in existing_rounds:
            import re
            m = re.search(r"(\d+)", rr["name"] or "")
            if m:
                round_by_number[int(m.group(1))] = rr["id"]

        for n in range(1, 39):
            if n not in round_by_number:
                sql(conn, "INSERT INTO rounds(name,open,description) VALUES(?,?,?)",
                    (f"Jornada {n}", True, "LaLiga 2026/2027"))
                rid = sql(conn, "SELECT id FROM rounds ORDER BY id DESC LIMIT 1").fetchone()["id"]
                round_by_number[n] = rid

        # Match source events by ESPN's week number. If week is absent, use the
        # exact home/away pair already present in our database as a safe fallback.
        existing_pairs = {}
        for row in sql(conn, "SELECT id,round_id,home,away FROM matches").fetchall():
            existing_pairs[(_norm_team(row["home"]), _norm_team(row["away"]))] = row["id"]

        for ev in events:
            home, away, raw = _espn_result(ev)
            ...
            try:
                ...
                week = None
            if not week or not 1 <= week <= 38:
                continue
            rid = round_by_number[week]
            kickoff = raw.get("date")
            # Store Madrid-local ISO without timezone; parse_local interprets it as Madrid.
            dt = None
            if kickoff:
                try:
                    dt = datetime.fromisoformat(kickoff.replace("Z", "+00:00")).astimezone(TZ)
                    kickoff = dt.strftime("%Y-%m-%dT%H:%M")
                except Exception:
                    pass
            ...
            if completed:
                try:
                    hg, ag = int(scores.get("home")), int(scores.get("away"))
            ...
            mid = existing_pairs.get(key)
            if mid:
                sql(conn, """UPDATE matches SET round_id=?,home=?,away=?,kickoff=?,actual=?,score=? WHERE id=?""",
                    (rid, home, away, kickoff, actual, score, mid))
            else:
                order = sql(conn, "SELECT COALESCE(MAX(match_order),0)+1 AS n FROM matches WHERE round_id=?", (rid,)).fetchone()["n"]
                sql(conn, """INSERT INTO matches(round_id,home,away,match_order,kickoff,actual,score)
                             VALUES(?,?,?,?,?,?,?)""",
                    (rid, home, away, order, kickoff, actual, score))
                mid = sql(conn, "SELECT id FROM matches ORDER BY id DESC LIMIT 1").fetchone()["id"]
                existing_pairs[key] = mid
        conn.commit()
    except Exception:
        conn.rollback()
    finally:
...
def get_round(conn, round_id=None):
    ...
    # Default view is the next round that has not started yet. This prevents
    # the automatic calendar import (which creates all 38 rounds) from
    # suddenly opening Jornada 38 on the home screen.
    ...
    candidates = []
    for rr in rounds:
        first = round_deadline(conn, rr["id"])
        if first and first > now:
            candidates.append((first, rr))
    if candidates:
        candidates.sort(key=lambda x: x[0])
        return candidates[0][1]

    # If no future kickoff is known, keep the latest configured round as a
    # safe fallback (for example while the external calendar is unavailable).
    return rounds[-1]
...
def render_matches(conn, r, user_id, editable=True):
    ...
    if deadline and can_edit:
        ...
        deadline_html = '<div class="deadline">⚠️ Calendario pendiente de sincronización automática. Se volverá a intentar en breve.</div>'
...
init_db()
# IMPORTANT: never call the external football feed while the Gunicorn worker is
# booting. Railway health checks must be able to reach /health immediately.
# Calendar/results synchronisation is performed automatically on normal requests.
